...
namespace sn::graphics
{
	...
	bool ConstantBuffer::Create(size_t size)
	{
		...
		sn::graphics::GetDevice()->CreateBuffer(buffer.GetAddressOf(), &desc, nullptr);
	...
	{
		sn::graphics::GetDevice()->SetConstantBuffer(buffer.Get(), data, desc.ByteWidth);
	}

	void ConstantBuffer::Bind(eShaderStage stage)
	{
		sn::graphics::GetDevice()->BindConstantBuffer(stage, mType, buffer.Get());
	}
